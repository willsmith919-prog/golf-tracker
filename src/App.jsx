      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-900 via-indigo-800 to-purple-900 p-6">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl p-6 mb-6">
            ...
            {activeSeason && activeSeason.events && activeSeason.events.length > 0 ? (
              <div className="space-y-3">
                <p className="text-sm text-gray-600 mb-3">Events will be loaded in real-time (coming in next chunk)</p>
                {activeSeason.events.map((eventId, index) => (
                  <div key={eventId} className="p-4 bg-gray-50 rounded-xl">
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="font-semibold text-gray-900">Event {index + 1}</div>
                        <div className="text-sm text-gray-600">Event ID: {eventId}</div>
                      </div>
                      {isCommissioner && (
                        <button
                          onClick={() => handleDeleteEvent(eventId, `Event ${index + 1}`)}
                          className="text-red-600 hover:text-red-700 text-sm font-semibold"
                        >
                          Delete
                        </button>
